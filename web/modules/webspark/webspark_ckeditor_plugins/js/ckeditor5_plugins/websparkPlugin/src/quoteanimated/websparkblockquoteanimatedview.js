...
export class WebsparkBlockquoteAnimatedFormView extends View {
  DEFAULT_TITLE = "";
  DEFAULT_TEXT = "";
  DEFAULT_STYLE = "";
  DEFAULT_HEADING = "";
  ...
  constructor(validators, locale) {
    ...
    this.citationDescriptionView = createInput(
  ...
  get title() {
    return this.titleView.children[1].element.value.trim();
  }

  set title(title) {
    this.titleView.children[1].element.value = title.trim();
  }
  ...
  setValues(values) {
    this.title = values?.title || this.DEFAULT_TITLE;
    this.text = values?.text || this.DEFAULT_TEXT;
    this.citationName = values?.citationName || this.DEFAULT_STYLE;
    this.citationDescription =
      values?.citationDescription || this.DEFAULT_HEADING;
  }
...
function prepareListOptions(options) {
  ...
  for (const option of options) {
    const def = {
      type: "button",
      model: new Model({
        commandName: "websparkBlockquoteAnimated",
